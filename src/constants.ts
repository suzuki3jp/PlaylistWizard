/**
 * The Name of the author.
 */
export const AUTHOR_NAME = "suzuki3jp";

/**
 * The URL of the author's GitHub profile.
 */
export const AUTHOR_GITHUB = "https://github.com/suzuki3jp";

/**
 * The URL of the author's X (Twitter) profile.
 */
export const AUTHOR_X = "https://x.com/suzuki3_jp";

/**
 * The URL of the source code repository.
 */
export const SOURCE_CODE_GITHUB = "https://github.com/suzuki3jp/playlistwizard";

/**
 * The URL of the YouTube Terms of Service page.
 * Used in the Terms and Privacy section.
 */
export const YOUTUBE_TOS = "https://www.youtube.com/t/terms";

/**
 * The URL of the Google Privacy Policy page.
 * Used in the Terms and Privacy section.
 */
export const GOOGLE_PRIVACY_POLICY = "https://policies.google.com/privacy";

/**
 * The URL of the Google Connections page.
 * Used in the Terms and Privacy section.
 */
export const GOOGLE_CONNECTIONS = "https://myaccount.google.com/connections";

/**
 * The URL of the Spotify Terms of Service page.
 * Used in the Terms and Privacy section.
 */
export const SPOTIFY_TOS = "https://www.spotify.com/legal/end-user-agreement/";

/**
 * The URL of the Spotify Privacy Policy page.
 * Used in the Terms and Privacy section.
 */
export const SPOTIFY_PRIVACY_POLICY =
    "https://www.spotify.com/legal/privacy-policy/";

/**
 * The URL of the Spotify Connections page.
 * Used in the Terms and Privacy section.
 */
export const SPOTIFY_CONNECTIONS = "https://www.spotify.com/account/apps/";

/**
 * The current version of the application.
 */
export const VERSION = `v${process.env.RELEASE}`;

/**
 * The default value of the target playlist ID.
 * Used in the MergeButton, CopyButton component in PlaylistController.
 */
export const DEFAULT = "default";
