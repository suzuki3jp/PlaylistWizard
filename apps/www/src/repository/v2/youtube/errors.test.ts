...
describe("YouTubeRepositoryError", () => {
  it("should extend RepositoryError", () => {
    const error = YouTubeRepositoryError.fromHttpStatus(401);
    expect(error).toBeInstanceOf(RepositoryError);
    expect(error).toBeInstanceOf(YouTubeRepositoryError);
  });
  ...
  describe("validationError", () => {
    it("should create error with VALIDATION_ERROR status", () => {
      const error = YouTubeRepositoryError.validationError("invalid field");
  ...
  describe("unknownError", () => {
    it("should create error with default message", () => {
      const error = YouTubeRepositoryError.unknownError();
      expect(error.status).toBe("UNKNOWN_ERROR");
      expect(error.code).toBe(0);
    ...
    it("should create error with custom message", () => {
      const error = YouTubeRepositoryError.unknownError("custom message");
      expect(error.status).toBe("UNKNOWN_ERROR");
      expect(error.code).toBe(0);
